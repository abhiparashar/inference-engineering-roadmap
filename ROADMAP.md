...
> **Detailed deep-dive (basics → advanced, plain words):** [`phases/phase-3/`](phases/phase-3/README.md) — 10 lessons: serving metrics, static/dynamic/continuous batching, queueing math behind p99, scheduling policies and admission control, honest load generation, and two builds (a dynamic-batched server and a tiny continuous-batching engine) with measured before/after numbers.
...
> **Detailed deep-dive (basics → advanced, plain words):** [`phases/phase-9/`](phases/phase-9/README.md) — 12 lessons: the four numbers that classify any serving workload and which Phase 2-8 techniques transfer, ranking inference where the model is 11% of a 10ms budget and fan-out tail amplification is the real problem, vision pipelines whose dominant stage is JPEG decode rather than the forward pass, streaming speech contracts measured in real-time factor, hardware chosen on cost per unit of useful work (TPU/XLA static shapes, Neuron buckets, CPU economics, Apple Silicon), ANN indexes tuned to a recall target with the RAM arithmetic that decides your bill, on-device packaging against megabyte/milliwatt/thermal budgets, model conversion as a gated build step with numerical tolerances, prompt injection as blast-radius engineering plus token-denominated rate limits and MIG-vs-time-slicing isolation, RAG/agentic pipelines with a budget, timeout and fallback per stage, and two builds: a CPU-vs-GPU serving shootout and a RAG service with guardrails you attack yourself.
